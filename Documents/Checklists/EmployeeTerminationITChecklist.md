...
# :speaking_head: Manager Inquiries

## :left_speech_bubble: Do you want to convert the employee’s mailbox to a Shared Mailbox?

### :mailbox: Shared Mailbox Features
...
:left_speech_bubble: **Do you want to send an automatic reply to all incoming emails?**
...
:left_speech_bubble: **Would you like this automatic reply to be sent to everyone or only company email addresses?**
...
## :left_speech_bubble: Do you need access to the employee’s OneDrive files?
...
## :left_speech_bubble: What accounts, services, or licenses need to be deleted for the user?

For a list of examples, see the [Services](#services) section below.

## :left_speech_bubble:  What company equipment did the employee have and has it been recovered?

For a list of examples, see the [Company Equipment](#company-equipment) section below.

# :arrows_counterclockwise: Device Recovery and Service Disconnection

## :computer: Company Equipment
...
## :iphone: Mosyle Device Management
...
## :telephone_receiver: Cellular Service
...
## 📥 Retain user data - eDiscovery hold (recommended)

### ✅ Create eDiscovery hold policy

While a user’s OneDrive can be recovered for 10 years from the data of deletion (this was changed from the default of 30 days to 10 years using the PowerShell command `Set-SPOTenant -OrphanedPersonalSitesRetentionPeriod 3650`), this should not be relied upon. Instead, an eDiscovery hold policy should be created for all terminated employees. *This will retain a user's **mailbox**, **OneDrive**, and **Teams** messages data.* Performing this process makes it easy to search a user's account for specific content which could be especially useful for compliance purposes or legal proceedings.

### Using PowerShell

Use the [New-eDiscoveryHold](https://github.com/lahma0/KarstUtilityScripts/blob/main/M365/Compliance/New-eDiscoveryHold.ps1) script located in the [KarstUtilityScripts](https://github.com/lahma0/KarstUtilityScripts) repo to quickly create an **eDiscovery hold** for the user:

> [KarstUtilityScripts](https://github.com/lahma0/KarstUtilityScripts) > [M365](https://github.com/lahma0/KarstUtilityScripts/tree/main/M365) > [Compliance](https://github.com/lahma0/KarstUtilityScripts/tree/main/M365/Compliance) > [New-eDiscoveryHold.ps1](https://github.com/lahma0/KarstUtilityScripts/blob/main/M365/Compliance/New-eDiscoveryHold.ps1)

### Using web GUI

- Navigate to the Microsoft Purview [eDiscovery](https://purview.microsoft.com/ediscovery/casespage) page and click the **Create case** button
...
## 📋 Retain user data - litigation hold (not recommended)

<details>
  <summary>Click to show/hide</summary>

> [!WARNING]
> Setting a litigation hold on the user's mailbox prevents completion of the [Delete user](#account-deletionconversion) workflow which is used to convert the user's mailbox to a Shared Mailbox and remove all licenses from the account. An [eDiscovery hold](#retain-data-using-an-ediscovery-hold-recommended) should be implemented instead. This will retain the user's mailbox/OneDrive/Teams data using Microsoft Purview.

Set a litigation hold on the user’s mailbox for 3650 days (10 years) to ensure retention for compliance/legal purposes, regardless of account deletion actions. Messages are held from the date they were received or created, not from when the hold was set.
...
If you delegated access to the user's OneDrive, the manager should get an email with the employee's OneDrive link, but this can take a long time or may not happen at all. It's probably best to just send the link yourself which follows this format:  

> <https://tmconcrete-my.sharepoint.com/personal/email_domain_tld/>

Example for jordan@karst.com:

> <https://tmconcrete-my.sharepoint.com/personal/jordan_karst_com/>

## 📅 Schedule follow-up

After converting a user's mailbox to a Shared Mailbox, check with the manager in 6 months about continued access. If needed, check again in another 6 months; if not, [delete the Shared Mailbox](#delete-a-shared-mailbox-and-stop-automatic-replies).

# ℹ️ Additional Information

## Delete a Shared Mailbox and stop automatic replies

- Navigate to [Microsoft 365 Admin Center - Shared Mailboxes](https://admin.microsoft.com/Adminportal/Home?#/SharedMailbox)
...
A deleted user's mailbox will be retained for the period specified in the Exchange retention policy in Microsoft Purview (currently 10 years). To recover a deleted user's entire mailbox, navigate to [Microsoft Purview Inactive Mailboxes](https://purview.microsoft.com/datalifecyclemanagement/inactivemailbox) page, select a user, and click **Export**.

## ⏱️ Set deleted OneDrive retention period

By default, a deleted user’s OneDrive is retained for 30 days before being sent to the recycling bin (at which point it will be deleted after 93 days). This value was already modified to 3650 days (10 years). To change it to a different value, use the following PowerShell script:
